...
public:
	...
	double Solve(double (*costmatrix)[MWM_CSP_MAX_POINTS], int (*res)[2],
...
double Matching::Solve(double (*costmatrix)[MWM_CSP_MAX_POINTS], int (*res)[2],
			std::vector<int>& free, std::vector<int>& outer, std::vector<int>& tip, std::vector<bool>& active,
			std::vector<int>& type, std::vector<int>& forest, std::vector<int>& root, std::vector<bool>& blocked,
			std::vector<double>& dual, std::vector<double>& slack, std::vector<int>& mate, std::vector<int>& forestList, std::vector<int>& visited)
{
...
double MinimumCostPerfectMatching(int n, double (*costmatrix)[MWM_CSP_MAX_POINTS], int (*res)[2])
{
	std::vector<double> slack;//slack associated to each edge, if slack[e] > 0, the edge cannot be used

	for (int i=0;i<n;i++)
		for (int j=i+1;j<n;j++)
			slack.push_back(costmatrix[i][j]);
...
static double greedy_edge_assignment(int num_points, double (*weights)[MWM_CSP_MAX_POINTS], bool* p_matching)
{
	int pair[32];
	double csp = 0.0;
	for (int i=0;i<num_points;i++)
	{
		...
		{
			double edge_weight = weights[i][j];
